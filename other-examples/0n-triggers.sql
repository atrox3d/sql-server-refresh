...
PRINT 'INFO |';
PRINT 'INFO | --- What is the structure of inserted/deleted tables? ---';
PRINT 'INFO | They are an exact mirror of the table the trigger is on.';
GO

CREATE OR ALTER TRIGGER trg_ShowUpdateStructure ON dbo.ScopeTest_Outer
AFTER UPDATE
AS
BEGIN
    SET NOCOUNT ON;
    PRINT 'INFO | --- Trigger Fired (UPDATE) ---';
    PRINT 'INFO | Structure of the "deleted" table (the old values):';
    SELECT * FROM deleted;

    PRINT 'INFO | Structure of the "inserted" table (the new values):';
    SELECT * FROM inserted;
END
GO

PRINT 'INFO | Updating a row to fire the trigger...';
UPDATE dbo.ScopeTest_Outer SET Data = 'Updated Data' WHERE ID = 100;
GO
